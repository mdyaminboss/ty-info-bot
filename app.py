...
def get_db():
    db_path = os.path.join(os.getcwd(), 'storage/tynex_host.db')
...
def init_db():
    ...
    if not db.execute('SELECT * FROM admin_settings WHERE id=1').fetchone():
        # ডিফল্ট অ্যাডমিন ইউজারনেম 'tyyamin' এবং পাসওয়ার্ড '1234' সেট করা হলো
        db.execute('INSERT INTO admin_settings (id, username, password) VALUES (1, "tyyamin", "1234")')
...
def create_app():
    app = Flask(__name__)
    app.config['SECRET_KEY'] = 'tynex_official_secure_key_999'
    ...
    def admin_browse_files(folder):
        if not session.get('admin_logged'): return redirect(url_for('admin_login'))
        return render_template('web/dashboard.html', user={'fname': 'Admin (TY YAMIN)'}, is_admin_view=True, admin_folder=folder)
    ...
    def server_command(folder):
        # Interactive command injector if needed
        return jsonify({'status': 'ok'})
